...
func newGetCodeID(ctx context.Context, store db.Store) *GetCodeLoader {
	return NewGetCodeLoader(GetCodeConfig{
		Wait:     10 * time.Millisecond,
		MaxBatch: 100,
		Fetch: func(id []int64) ([]*model.Code, []error) {
			c, err := store.GetCode(ctx, int64(id[0]))
			if err != nil {
				return nil, []error{fmt.Errorf("get a code dataloader error: %v", err)}
			}
			...
			return []*model.Code{convertCode}, nil
		},
	})
}
